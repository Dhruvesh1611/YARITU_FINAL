...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchAll() {
      try {
        // Fetch stores, hero, and trending in parallel
        const [storesRes, heroRes, trendingRes] = await Promise.all([
        ...
        if (storesRes) {
          ...
          if (storesJson && storesJson.success && Array.isArray(storesJson.data)) {
        ...
        if (heroRes) {
          ...
          if (heroJson && heroJson.success && Array.isArray(heroJson.data)) {
            setHeroItems(heroJson.data);
          }
        }

        // Trending
        if (trendingRes) {
          const trendingJson = await trendingRes.json().catch(() => null);
          if (trendingJson && trendingJson.success && Array.isArray(trendingJson.data)) {
            setTrendingVideos(trendingJson.data);
          }
        }
      } catch (error) {
    ...
    fetchAll();
  }, []); // Is khaali array ko na hatayein, yeh zaruri hai
  ...
  }, []); // Removed isHeroPaused from dependency array 
  ...
  return (
    <>
      ...
      >
        ...
        {/* Background image rendered with next/image fill to ensure desktop and mobile behave the same */}
        <AnimatePresence>
          <motion.div
